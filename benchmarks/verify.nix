let
  ...
  check =
    ...
    else if builtins.isList a && builtins.isList b && srt a == srt b then
      {
        ok = true;
        msg = "${name}: OK (order differs)";
      }
    else
      {
        ok = false;
  ...
  checks = [
    ...
    (check "packages names" (srt (map (p: p.name) zen.packages)) (srt (map (p: p.name) np.packages)))
  ];
