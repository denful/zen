...
in
{
  advanced = {
    ...
    # §6) — defs at differing priorities never co-reach a strategy (the lower
    # number wins outright). To exercise the `conflict` NEGOTIATION (its actual
    # intent), the two defs must be SAME-priority so both survive the filter and
    # the strategy genuinely sees a 2-def conflict. (Pre-R3 these used 50/100 and
    # relied on the strategy doing its own cross-prio selection, which R3's filter
    # subsumes.)
    ...
    # --- zen.reconcile (R5: deterministic fold over a LIST, spec §4.2) ---
    # (Ported from the old stream-`reconcile`, which R5 superseded with the
    #  §4.2 list-fold surface. `reconcile init step coll` folds `coll` in order
    #  and returns the FINAL accumulator; here the accumulator threads the next
    #  port and accumulates the per-claim assignments.)
